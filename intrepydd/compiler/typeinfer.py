...
class TypeInferer(ast.NodeVisitor):
    def __init__(self):
        if glb.args.verbose:
            self.logger = Logger('typeinfer_log.xml')
        else:
            self.logger = Logger(os.devnull)
    ...
    def do_node(self, N):
        self.logger.open_tag('Node')
        self.logger.print(ast.dump(N))
        ...
        if N.value.id in self.typemap:
            ...
            if (isinstance(ty, mytypes.NpArray) and ty.ndim != -1):
                ...
                if ty.ndim != ndim_found:
                    msg = 'annotated # array dims = %d while found = %d' % (
                        ty.ndim, ndim_found)
                    #glb.exit_on_node(N, msg)
    ...
    def do_Subscript(self, N):
        ...
        # Arrays can be accessed without range/shape
        libfuncs.add_function_module('arraysub')
    ...
    def eval_type_comment(self, s: str):
        ...
        if s == 'int' or s == 'int32':
            ...
            raise Exception('Unrecognized type comment: ' + s)
    ...
    def do_tuple_assign(self, N: ast.Assign):
        ...
        if len(value_tp) != len(var_tp):
            raise SyntaxError(
                "# of elements in the left and right don't match in assignment")
 ...
 #       print(N.type)
    ...
    def do_Return(self, N: ast.Return):
        ...
        if self.cur_func.ret_type == mytypes.void:
            ...
            if not N.value.type.is_subtype_of(self.cur_func.ret_type):
                m = 'Unmatched return types. \nPreviously: ' + \
                    str(self.cur_func.ret_type) + '\n'
