...
def parse_args():
    ...
    parser.add_argument('-blas', dest='blas', action='store_true', help='Use BLAS calls')
    parser.add_argument('-no-blas', dest='blas', action='store_false', help='Do not use BLAS calls')
    parser.set_defaults(blas=True)
...
#    parser.add_argument('-g', default=False, action='store_true', help='Enable debug mode')
    ...
    parser.add_argument('-smtti', default=False, action='store_true', help='Use SMT type inference')
    parser.add_argument('-smtti-max-smt-models', type=int, default=1000, help='Max number of SMT models to use')
    parser.add_argument('-smtti-count-only', action='store_true', help='Count the number of SMT models only')
    parser.add_argument('-smtti-use-annot', action='store_true', help='Use existing type annotations')
    parser.add_argument('-smtti-use-bounds', action='store_true', help='Use manual type bounds')
    parser.add_argument('-smtti-same-bitwidth', action='store_true', help='Use same bitwidth in parameter types')
    parser.add_argument('-smtti-use-ml', action='store_true', help='Use machine learning type inference')
    parser.add_argument('-smtti-ml-auto', action='store_true', help='Automatically adjust the number of ML constraints to be satisfied')
    parser.add_argument('-smtti-ml-auto-min-vers', type=int, default=0, help='Min num versions in auto mode')
    parser.add_argument('-smtti-ml-ratio', type=float, default=1., help='Ratio of ML constraints to be satisfied')
    parser.add_argument('-smtti-ml-model', choices=['rnn', 'tf', 'freq', 'chatgpt'], default='tf', help='ML type inference model')
    parser.add_argument('-smtti-vocab', type=str, default=None, help='Vocabulary path')
    parser.add_argument('-smtti-tf-encoder', type=str, default='Salesforce/codet5-large-ntp-py',
                        help='Pretrained Transformer encoder name')
    parser.add_argument('-smtti-model-path', type=str, default=None, help='Model path')
    parser.add_argument('-smtti-beam-size', type=int, default=32, help='Beam size for beam search')
    parser.add_argument('-smtti-max-type-length', type=int, default=8, help='Max type sequence length')
    parser.add_argument('-smtti-top-k', type=int, default=20, help='Number of predictions to use')
    parser.add_argument('-smtti-wrapper-memoize', default=False, action='store_true',
                        help='Enable memoization of dispatched functions in the generated Python wrapper')
    parser.add_argument('-smtti-wrapper-verbose', default=False, action='store_true',
                        help='Enable verbose mode of the generated Python wrapper')

    parser.add_argument('-numba', default=False, action='store_true', help='Use numba as backend')
    parser.add_argument('-numba-opt', default=False, action='store_true', help='Use numba as backend with intrepydd optimizations')
    parser.add_argument('-python', default=False, action='store_true', help='Use python backend')
...
class CppModule(object):
    ...
    def do_pymodule_header(self):
        ...
        cxxflags = ['-O3', '-std=c++14', '-Wno-unused-but-set-variable', '-Wno-unused-variable', '-Wno-unknown-pragmas']
    ...
    def do_py_footer(self):
        if not self.pymodule:
            return
        m = get_module_name()
        self.writeln('PYBIND11_MODULE(%s, m) {' % m)
        self.writeln('  m.doc() = "auto-compiled c++ extension";')
        for F in self.funcs:
            fn = F.name
...
def get_module_name(original=False):
    module_name = os.path.basename(get_module_fullname())
    if not original:
        if args.smtti:
            module_name += '_smtti'
    return module_name
