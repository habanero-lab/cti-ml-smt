...
class ModuleGen(ast.NodeVisitor):
        ...
        if 'licm_array' in glb.options and glb.options['licm_array']:
            if hasattr(N, 'inv_array_dim'):
                for array_id in N.inv_array_dim:
                    ...
                    if array_id in args:
                        continue
                    self.gen_data_ptr_decls(
                        array_id, N.inv_array_dim[array_id])
            ...
            #     ty = self.get_local_type(v)
                if isinstance(ty, mytypes.NpArray):
                    ...
                    s += '%s* %s_data_ptr_pydd = %s.mutable_data();\n' \
                        % (etype, v, v)
                    if ty.ndim == 2:
                        if (v, 1) not in self.cur_func_shape_decls:
                            self.cur_func_shape_decls.add((v, 1))
                            s += '%s %s_shape1_pydd;\n' % (mytypes.int64, v)
                        s += '%s_shape1_pydd = %s.shape(1);\n' \
                            % (v, v)
    ...
    def gen_data_ptr_decls(self, name, ndim):
        ...
        for d in range(1, ndim):
            if (name, d) in self.cur_func_shape_decls:
                continue
            self.cur_func_shape_decls.add((name, d))
    ...
    def gen_data_ptr_assign(self, name, ndim):
        ...
        for d in range(1, ndim):
            arr_shape_d = '%s_shape%i_pydd' % (name, d)
            if (name, d) not in self.cur_func_shape_decls:
                self.cur_func_shape_decls.add((name, d))
                s += '%s %s;\n' % (mytypes.int64, arr_shape_d)
            s += '%s = %s.shape(%i);\n' % (arr_shape_d, name, d)
        return s
    ...
    def eval_Assign(self, N: ast.Assign):
        ...
        for target in N.targets:
            ...
            # Need some interpretation
                ...
                s = self.eval_get_or_set_item(
                    self.eval(target.value), target, self.eval(N.value))
    ...
    def eval_name_constant(self, N: ast.NameConstant):
        ...
        if self.is_range_for(N):
            ...
            if glb.args.slice_opt and astutils.is_name(iter):
                s += '%s = pydd::getitem%s(%s, %s);\n' % (target,
                                                          self.get_dimension_suffix(iter), iter, index)
            else:
...
for now"
        ...
        s = 'std::unordered_map<%s, %s>{' % (N.type.key, N.type.value)
        s += '}'
        return s
    ...
    def eval_Call(self, N: ast.Call):
        ...
        if isinstance(N.func, ast.Name):
            ...
            if glb.is_user_defined_func(funcname):
                ...
                ns = 'pydd'
                if funcname == 'heapinit_empty':
                    t = N.type
                    assert isinstance(t, mytypes.Heap)
                    type_params = f'<{t.key.get_cpp_name()},{t.val.get_cpp_name()}>'
            else:
    ...
    def eval_List(self, N: ast.List):
        s = 'std::vector<%s>{' % N.type.etype
        s += ', '.join(list(map(lambda x: self.eval_node(x), N.elts)))
        s += '}'
        return s

    def eval_Tuple(self, N: ast.Tuple):
        if N.type.etypes:
            et = N.type.etypes[0]
        else:
            et = 'int64_t'
    ...
    def eval_BinOp_raw(self, op, left, right):
        ...
        if isinstance(op, ast.Add):
            s += 'pydd::add(%s, %s)' % (lhs, rhs)
        elif isinstance(op, ast.Sub):
            s += 'pydd::sub(%s, %s)' % (lhs, rhs)
        elif isinstance(op, ast.Mult):
            s += 'pydd::mul(%s, %s)' % (lhs, rhs)
        elif isinstance(op, ast.Div):
            s += 'pydd::div(%s, %s)' % (lhs, rhs)
        elif isinstance(op, ast.FloorDiv):
            s += 'pydd::floor_div(%s, %s)' % (lhs, rhs)
    ...
    # def eval_BinOp_array(self, op, lhs, rhs):
    #     s = ''
    #     if isinstance(op, ast.Add):
    #         s += 'pydd::add(%s, %s)' % (lhs, rhs)
    #     elif isinstance(op, ast.Sub):
    ...
    # def eval_BinOp_scalar(self, op, lhs, rhs):
    #     s = ''
    #     if isinstance(op, ast.Add):
    #         s += '%s + %s' % (lhs, rhs)
    #     elif isinstance(op, ast.Sub):
    ...
    def eval_Assert(self, N: ast.Assert):
