...
ew_funcs = {
    ...
    # Map to BinOp
    'add': ast.Add, 'sub': ast.Sub, 'mul': ast.Mult, 'div': ast.Div, 'floor_div': ast.FloorDiv,
    ...
    'pow': 2, 'log': [1, 2], 'maximum': 2, 'logical_xor': 2,
}
...
class ArrayOptimization(ast.NodeVisitor):
    ...
    def scalarize_array_op(self, expr: ast.expr, src_spm: str,
        ...
        if is_scalar_var(expr, self.curr_func):
            ...
            if isinstance(ewf, int) or isinstance(ewf, list):
                if isinstance(ewf, int):
                    assert len(expr.args) == ewf
                else:
                    assert len(expr.args) in ewf
                for i in range(len(expr.args)):
    ...
    def dense_opt_recurse(self, stmt: ast.stmt, expr: ast.expr, opt_list: list):
        ...
        elif is_var_with_type(expr, self.curr_func, mytypes.SparseMat):
            dim = self.curr_func.typemap[expr.id].ndim
    ...
    def generate_compatibility_check(self, target_call: ast.Call):
        ...
        if len(arrays) >= 2:
            ...
            check = new_call('compatibility_check', args0, mytypes.int64_list)
            return check
        return None

    def generate_init_dense(self, src_var: str, tar_var: str, iv: list, init_shape_var: Optional[str] = None):
        if init_shape_var is not None:
            shapes = ast.Name(init_shape_var, ast.Load())
        else:
        ...
        if isinstance(et, mytypes.IntType):
            ...
            assert False
        etype = new_call(ef, [])
        assign = new_assign(tar_var, new_call('empty', [shapes, etype]))
        assign.no_hoist = True
        return assign
            ...
            if isinstance(ewf, int) or isinstance(ewf, list):
                ...
                if verbose_level >= 1:
                    if isinstance(ewf, int):
                        ewf = [ewf]
                    if nargs not in ewf:
                        print('[Warning] Unmatched # args in for func:', expr.func.id)
    ...
    def dense_opt_codegen(self, opt_info: OptInfo):
        ...
        if check:
            init_shape_var = new_var(mytypes.int64_list, self.curr_func)
            stmts.append(new_assign(init_shape_var, check))
        else:
            init_shape_var = None
        ...
        if (not opt_info.full_expr
            ...
            init = self.generate_init_dense(opt_info.src_var, opt_info.tar_var, idxvec, init_shape_var)
            stmts.append(init)
